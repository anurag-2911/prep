...
func(this *Room)scheduleInterview(candidates []Candidate){
	for _, cand := range candidates {
		duration:=cand.duration
		minroom:=0
		maxtime:=math.MaxInt
		for i, room := range this.roomNumber {
			this.roomNumber[i].bookedTime< maxtime
			if(!room.booked){
				this.roomNumber[i].bookedTime = duration

			}else{
				this.roomNumber[i].bookedTime=this.roomNumber[i].bookedTime+duration
			}
			break
		}
...
func(this *Room)getBestRoom(rooms []Room){

}
func(this *Room)getRoomTime(id string)(string,string,string){
	for _, room := range this.roomNumber {
